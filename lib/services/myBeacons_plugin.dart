...
class BeaconsPlugin {
  ...
  static Future<void> stopMonitoring() async {
    channel.invokeMethod('stopMonitoring').then((result) {
      printDebugMessage(result, 2);
  ...
  static Future<void> listenToBeacons(StreamController controller, int index) async {
    streamSubscription = event_channel.receiveBroadcastStream(index).listen((dynamic event) {
      controller.add(event);
    }, onDone: () {
      printDebugMessage('Received onDone : complete and finish ...  ', 1);
